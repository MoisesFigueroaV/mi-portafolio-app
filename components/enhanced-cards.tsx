        ...
        <div className="space-y-2 sm:space-y-1">
          ...
          <div className="flex flex-wrap gap-2 sm:gap-1.5 pt-3 sm:pt-1">
            {project.tags?.slice(0, 3).map((tag) => (
              <span
                key={tag}
                className="px-3 py-1.5 sm:px-2 sm:py-0.5 text-sm sm:text-[10px] text-white/60"
              >
                {tag}
              </span>
            ))}
  ...
  return (
    ...
    >
      <div className="flex-grow">
        <h3 className="text-lg sm:text-sm font-bold leading-tight group-hover:text-white transition-colors text-white/90">
          {post.title[language]}
        </h3>
        {post.excerpt && (
          <p className="mt-2 sm:mt-1 line-clamp-2 text-base sm:text-xs leading-relaxed text-white/60">{post.excerpt[language]}</p>
        )}
      </div>
      {post.readingTime && <span className="flex-shrink-0 ml-4 text-base sm:text-xs text-white/60">{post.readingTime}</span>}
    </button>
  )
}